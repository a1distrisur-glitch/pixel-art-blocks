...
export type EditorTool = "place" | "erase" | "text" | "move" | "pipette";
...
export function useBrickEditor(initialWidth = 32, initialHeight = 32) {
  ...
  const hasContent = hasBricks || textOverlays.length > 0;

  const exportAsPng = useCallback(() => {
    const defaultName = `Pixcool-art ${timestamp()}.png`;
    const scale = 2;
    ...
    const brandH = 28;
    const legendH = legendRows.length > 0 ? legendPadding * 2 + 20 + rows * legendItemH + 8 + brandH : brandH + legendPadding * 2;
  ...
  }, [gridWidth, gridHeight, bricks, colors, textOverlays]);

  const exportPieceList = useCallback(() => {
    const sizeLabel = (s: BrickSize) => s === 1 ? "1x1" : `1x${s}`;
    const counts: Record<string, { color: string; colorName: string; size: BrickSize; count: number }> = {};
    for (const b of bricks) {
      const cName = colors.find((c) => c.value === b.color)?.name ?? b.color;
    ...
    let csv = "\uFEFFColor,Hex,Tamano,Cantidad\n";
    for (const r of rows) {
    ...
    saveWithPicker(blob, `lista-de-piezas ${timestamp()}.csv`, "Archivo CSV", { "text/csv": [".csv"] });
  }, [bricks, colors]);

  const saveProject = useCallback(() => {
    const fileName = `pixcool-art-project ${timestamp()}.json`;
  ...
  }, [gridWidth, gridHeight, bricks, textOverlays, colors, selectedColor, selectedSize, orientation, referenceImage, imageOpacity, imageVisible, imageLocked, imageTransform]);
