...
// SVG path generator for the same shapes (used for vector previews and overlays)
export function renderShapeSVGPath(
  shape: ShapeType,
  fillMode: ShapeFillMode,
  x: number,
  y: number,
  ...
  switch (shape) {
    ...
    case "triangle":
      return { path: `M ${cx} ${y} L ${x + w} ${y + h} L ${x} ${y + h} Z`, isClosed: true };
    case "rightTriangle":
